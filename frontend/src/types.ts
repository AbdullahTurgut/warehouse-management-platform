...
export type Pallet = { id: number; code: string; productId: number; sku: string; productName: string; quantity: number; location: Location | null; status: 'ACTIVE' | 'DISPATCHED'; version: number; createdAt: string; receivingSessionId: number | null; receivingSessionCode: string | null; firstPutAwayAt: string | null };
...
export type ReceivingSession = { id: number; code: string; warehouseId: number; warehouseName: string; receivingLocationId: number; receivingLocationCode: string; deliveryNote: string; supplier: string; expectedPalletCount: number | null; note: string; status: 'OPEN' | 'COMPLETED'; openedBy: string; openedAt: string; completedAt: string | null; version: number; receivedCount: number; putAwayCount: number; pendingCount: number; dispatchedBeforePutAwayCount: number };
export type ReceivingSessionDetail = { session: ReceivingSession; pallets: Pallet[]; queue: Pallet[] };
